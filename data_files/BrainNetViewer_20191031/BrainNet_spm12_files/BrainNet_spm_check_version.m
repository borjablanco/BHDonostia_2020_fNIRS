function varargout = BrainNet_spm_check_version(tbx,chk)
% Check a version number against a Toolbox version
%
% FORMAT tbx = BrainNet_spm_check_version
% tbx    - toolbox name {'matlab','octave',...}
%
% FORMAT v = BrainNet_spm_check_version(tbx)
% tbx    - toolbox name {'matlab','octave','signal',...}
%
% v      - version number {string}
%
% FORMAT status = BrainNet_spm_check_version(tbx,chk)
% tbx    - toolbox name {'matlab','octave','signal',...}
...
% status = BrainNet_spm_check_version('matlab','7');
...
% status = BrainNet_spm_check_version('matlab','7.2');
...
% status = BrainNet_spm_check_version('matlab','6.5.0');
% returns status == 1     : major + minor + release revision is later
%                           for MATLAB
% The statement ( BrainNet_spm_check_version('matlab','6.5.0') > 0 ) is true for
...
% Darren Gitelman
% $Id: BrainNet_spm_check_version.m 6156 2014-09-05 17:34:53Z guillaume $
...
if strcmpi(tbx,'matlab') && strcmpi(BrainNet_spm_check_version,'octave')
    varargout = {1}; % hack
    return;
end
