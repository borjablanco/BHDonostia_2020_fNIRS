function [strout,R2] = BrainNet_spm_str_manip(strin,options)
% miscellaneous string manipulation options
% FORMAT string_out = BrainNet_spm_str_manip(string_in,options)
...
%__________________________________________________________________________
%
% This function is now deprecated, use BrainNet_spm_file when possible instead.
%__________________________________________________________________________
% Copyright (C) 1996-2011 Wellcome Trust Centre for Neuroimaging

% John Ashburner
% $Id: BrainNet_spm_str_manip.m 4444 2011-08-26 16:47:14Z guillaume $


% persistent runonce
% if isempty(runonce)
%     warning('BrainNet_spm_str_manip is deprecated. Use BrainNet_spm_file instead.');
%     runonce = 1;
% end
...
while ~isempty(options)
    ...
    for i=1:numel(strout)
        ...
        %------------------------------------------------------------------
            ...
            if l > 4
                if ismember(str((l-3):l),...
                        {'.img','.hdr','.nii','.gii','.mat'})
                    str = BrainNet_spm_str_manip(str, 'r');
                end
            end
        ...
        %------------------------------------------------------------------
            str = BrainNet_spm_select('CPath',str);
