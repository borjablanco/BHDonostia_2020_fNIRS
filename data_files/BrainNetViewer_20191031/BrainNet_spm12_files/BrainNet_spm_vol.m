function V = BrainNet_spm_vol(P)
% Get header information for images
% FORMAT V = BrainNet_spm_vol(P)
...
%       V.dt    - A 1x2 array.  First element is datatype (see BrainNet_spm_type).
...
%
% Note that BrainNet_spm_vol can also be applied to the filename(s) of 4-dim
...
% John Ashburner
% $Id: BrainNet_spm_vol.m 5958 2014-04-16 17:13:41Z guillaume $
...
elseif iscell(P)
    V = cellfun(@BrainNet_spm_vol,P, 'UniformOutput',false);
    
else
    V = BrainNet_spm_vol;
    cnt = 0;
    for i=1:size(P,1)
        v = BrainNet_spm_vol_hdr(deblank(P(i,:)));
...
%==========================================================================
% function V = BrainNet_spm_vol_hdr(p)
%==========================================================================
function V = BrainNet_spm_vol_hdr(p)
[pth,nam,ext,n] = BrainNet_spm_fileparts(p);
p = fullfile(pth,[nam ext]);
n = str2num(n);
if ~BrainNet_spm_existfile(p)
...
switch ext
    ...
    case {'.img','.IMG'}
        if ~BrainNet_spm_existfile(fullfile(pth,[nam '.hdr'])) && ...
           ~BrainNet_spm_existfile(fullfile(pth,[nam '.HDR']))
    ...
    case {'.gz','.GZ'}
        ...
        try
            ...
            V = BrainNet_spm_vol_hdr([tmpname n]);
            for i=1:numel(V)
                V(i).dat   = BrainNet_spm_read_vols(V(i));
                V(i).private.dat.fname = BrainNet_spm_file(p,'ext','');
                V(i).fname = p;
                V(i).dt(1) = 64;
                V(i).pinfo = [1 0 0]';
            end
...
V = BrainNet_spm_vol_nifti(p,n);
    
if isempty(n) && length(V.private.dat.dim) > 3
    V0(1) = V;
    for i = 2:V.private.dat.dim(4)
        V0(i) = BrainNet_spm_vol_nifti(V.private, i);
    end
    V = V0;
end
