function s = BrainNet_spm_mesh_detect(F)
% True for valid representation of a mesh
% FORMAT s = BrainNet_spm_mesh_detect(F)
...
% Guillaume Flandin
% $Id: BrainNet_spm_mesh_detect.m 5065 2012-11-16 20:00:21Z guillaume $

s = false;

if iscellstr(F) || ischar(F)
    F = cellstr(F);
    [p,n,e] = BrainNet_spm_fileparts(F{1});
    if strcmpi(e,'.gii')
        s = true;
    end
elseif isstruct(F)
